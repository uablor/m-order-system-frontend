const k=/\d/g;function h(e){if(!e||typeof e!="string")return"";const n=e.match(k);return n?n.join(""):""}function m(e){return!e||e.length<8?!1:/^\d+$/.test(e)}function f(e,n){const s=`https://wa.me/${e}`;return n&&n.trim()?`${s}?text=${encodeURIComponent(n.trim())}`:s}function g(e){const n=(typeof window<"u"?window.location.origin:"").toString().trim().replace(/\/$/,""),s="/customer/item-arrived";if(!e||!e.includes("?"))return`${n}${s}`;const c=e.indexOf("?"),l=e.substring(c),a=`${n}${s}${l}`;return!a.startsWith("http://")&&!a.startsWith("https://")?`https://${a}`:a}function w(e={}){const{defaultMessage:n="",openInNewTab:s=!0}=e,c={en:{base:`Hello {customerName},
...
${i}`:r.message;const{template:o}=r;if(!o)return n;const p=o.customerName??(t==="en"?"Customer":t==="th"?"ลูกค้า":"ລູກຄ້າ"),N=o.orderNumbers?.length?t==="en"?`Orders #${o.orderNumbers.join(", #")}`:t==="th"?`ออเดอร์ #${o.orderNumbers.join(", #")}`:`ອໍເດີ #${o.orderNumbers.join(", #")}`:o.orderNumber!=null?t==="en"?`Order #${o.orderNumber}`:t==="th"?`ออเดอร์ #${o.orderNumber}`:`ອໍເດີ #${o.orderNumber}`:t==="en"?"your order":t==="th"?"ออเดอร์ของคุณ":"ອໍເດີຂອງທ່ານ",d=u.base.replace("{customerName}",p).replace("{orderPart}",N);if(i){const b=`${i}`,$=u.withLink.replace("{link}",b);return d+$}else return d+u.noLink};return{formatPhoneForWhatsApp:h,isValidWhatsAppPhone:m,getWhatsAppUrl:f,openWhatsAppChat:r=>{const i=h(r.phone);if(!m(i))return!1;const t=l(r),u=f(i,t);return s?window.open(u,"_blank","noopener,noreferrer"):window.location.href=u,!0}}}export{w as u};
