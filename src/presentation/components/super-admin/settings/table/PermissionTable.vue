<template>
  <div class="sa-access-table">
    ...
    <div v-else class="permissions-mobile">
      <a-collapse class="permissions-collapse" :expand-icon="expandCollapseIcon">
        <a-collapse-panel v-for="p in permissions" :key="p.id">
          ...
          <div class="card-detail">
            <div class="detail-row">
              ...
              <span class="detail-val">{{ formatDate(p.createdAt) }}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">Updated At</span>
              <span class="detail-val">{{ formatDate(p.updatedAt) }}</span>
            </div>
            <div class="detail-row action-row">
              <a-button type="primary" ghost size="small" class="action-btn" @click="$emit('edit', p)">{{ $t('common.edit') }}</a-button>
...
/* ===== Mobile card list ===== */
.permissions-mobile { display: flex; flex-direction: column; }
.permissions-collapse { background: transparent !important; border: none !important; }
.permissions-collapse :deep(.ant-collapse-item) {
...
.permissions-collapse :deep(.ant-collapse-header) { padding: 14px 14px !important; align-items: center !important; }
.permissions-collapse :deep(.ant-collapse-content) { background: transparent !important; border-top: 1px solid rgba(148, 163, 184, 0.18) !important; }
.permissions-collapse :deep(.ant-collapse-content-box) { padding: 0 14px 14px !important; }
...
.detail-val { color: #0f172a; font-weight: 600; text-align: right; word-break: break-all; }
