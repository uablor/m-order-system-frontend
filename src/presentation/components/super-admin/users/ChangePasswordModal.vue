  ...
  >
    <div class="change-password-form">
      <a-form
        ...
        @finish="handleSubmit"
      >
        <a-form-item :label="$t('users.currentPassword')" name="currentPassword">
          <a-input-password
        ...
        <a-form-item :label="$t('users.newPassword')" name="password">
          <a-input-password
        ...
        <a-form-item :label="$t('users.confirmNewPassword')" name="confirmPassword">
          <a-input-password
...
const rules = computed(() => ({
  currentPassword: [
    { required: true, message: t('users.currentPasswordRequired') },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await client.putOrPatch(
      `${API_ENDPOINTS.USERS.BASE}/${props.user.id}/change-password-by-id`,
      {
    ...
    message.success(t('users.changePasswordSuccess'));
    emit('success');
    visible.value = false;
    resetForm();
  } catch (error: any) {
    console.error('Change password error:', error);
    const errorMessage = error?.response?.data?.message || t('users.changePasswordError');
