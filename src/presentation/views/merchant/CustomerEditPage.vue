  ...
  <MerchantLayout>
    <div class="customer-form-page">
      <a-breadcrumb class="mb-3">
        ...
        <a-breadcrumb-item>{{ $t('merchant.customers.modal.editTitle') }}</a-breadcrumb-item>
      </a-breadcrumb>

      <div class="page-head">
        <div>
          <div class="text-3xl font-semibold text-slate-900">{{ $t('merchant.customers.modal.editTitle') }}</div>
...
const { loading, updateCustomer } = useMerchantCustomers();
...
const submit = async () => {
  ...
  const ok = await updateCustomer(id, payload);
