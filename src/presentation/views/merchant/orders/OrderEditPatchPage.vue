<template>
  <MerchantLayout>
    ...
    <EditOrderContent
      ...
      :initial-buy-rate="initialBuyRate"
...
import EditOrderContent from '../../../components/merchant/editOrder/EditOrderContent.vue';
...
import type { Order, ExchangeRateSnapshot } from '@/domain/entities/user.entity';
...
const contentRef = ref<InstanceType<typeof EditOrderContent>>();
...
const initialBuyRate = ref<ExchangeRateSnapshot | null>(null);
const initialSellRate = ref<ExchangeRateSnapshot | null>(null);
...
onMounted(async () => {
  ...
  try {
    ...
    initialBuyRate.value = mapped.initialBuyRate ?? null;
    initialSellRate.value = mapped.initialSellRate ?? null;
  } catch (err) {
    error.value = true;
